...
class TableRef < QueryModel
  ...
  def compute_column_metadata(columns)
    @columns.each do |col|
      ...
      columns[metadata.name] = metadata if metadata
    end
    @joins.each do |join|
...
class ColumnRef < QueryModel
  ...
  def compute_metadata(table)
    metadata = ColumnMetadata.new @engine

    # the full name of the selected column, including alias and table prefix
    metadata.name = [table.prefix.presence, @alias.presence || name].compact.join
    ...
    metadata.type = sql_type_metadata.type
